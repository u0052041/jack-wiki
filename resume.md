...
7 年 Python 後端開發經驗，專注於高流量系統的架構設計與效能優化。曾於 DAU 50 萬的影音平台主導核心功能開發，具備資料庫與快取層調校、金流串接及第三方服務整合的實戰經驗。
具備 CI/CD 實務背景，並透過 Side Project 系統性建構 AWS/Terraform 基礎設施與 Kubernetes 部署經驗，目標轉往 DevOps 發展。
...
- 獨立串接第三方直播服務，負責從技術評估、API 整合到上線維護，包含直播間充值與消費並確保交易資料一致性
...
以 IT 職務入職後自學轉型後端開發，獨立從 0 到 1 建構內部管理系統。撰寫 integration test 保障核心功能品質，修復 N+1 query 問題提升效能，以 Python + Selenium 實作自動化流程取代人工重複作業。

## DevOps 實踐

透過 Side Project 從零建構 AWS 基礎設施與 CI/CD 流程，系統性累積 DevOps 相關實作經驗。
- 使用 Terraform 管理 VPC、Subnet、ALB、ACM、ECS 等 AWS 資源，實踐 Infrastructure as Code
- 整合 ACM wildcard certificate 與 ALB，完成 HTTPS 流量配置
- 建置 Jenkins Pipeline，搭配 ECS Agent 實現動態 build agent 的 CI/CD 流程
- 部署應用至 EKS，撰寫 Kubernetes manifests 處理 Ingress、Service、Deployment 配置
