...
| [ACM / ALB / Domain 完整串接流程](infra/acm-alb-notes) | 從憑證申請到 DNS 上線，完整走一次 ACM 驗證、ALB 掛證書與 Cloudflare CNAME |
| [Terraform 實戰筆記](infra/terraform-notes) | 日常工作流、state 管理、import/moved、CI/CD 常見做法與安全操作清單 |
| [AWS VPC Networking](infra/aws-vpc-networking-notes) | subnet 設計、NAT GW、VPC Endpoint、ALB vs NLB、Session Manager |
| [Terraform + Jenkins 學習筆記](infra/jenkins/deploy-notes) | 共用 VPC networking 層、Remote State、ACM、ALB、ECR cross-platform build、IAM for EKS |
| [EKS 筆記](infra/aws-eks-notes) | IAM Roles、Node Group、IRSA、OIDC、ALB Controller、Helm、Prod Best Practices |
| [EKS 部署筆記](infra/k8s/deploy-notes) | 專案結構、Cluster 設定決策、Jenkins 整合、ALB Controller Pipeline |
